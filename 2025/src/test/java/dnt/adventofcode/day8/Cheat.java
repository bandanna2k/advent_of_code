...
public class Cheat
{
    public static void main(String[] args) throws IOException
    {
        ...
        if (example)
        {
            br = new BufferedReader(new InputStreamReader(Day8Part1Test.class.getResourceAsStream("/day8test.txt")));
            numConnections = 10;
        }
        else
        {
            br = new BufferedReader(new InputStreamReader(Day8Part1Test.class.getResourceAsStream("/day8real.txt")));
            numConnections = 1000;
        }
        ...
        for (Distance dist : distances)
        {
            ...
            String join = circuitHeads.values().stream().map(v -> String.valueOf(v)).collect(Collectors.joining(","));
            ...
            { // add two to one's circuit
                dist.two.head = head1;
                circuitHeads.put(head1, size1 + 1);
            }
            else if (size1 == null && size2 != null)
            { // add one to two's circuit
                dist.one.head = head2;
